...
export async function GET(req: NextRequest) {
  ...
  if (!session.isLoggedIn || !session.accsoftCookies) {
    ...
    return NextResponse.redirect(new URL("/api/auth/logout?type=accsoft&next=/accsoft/no-dues", req.url));
  }
  ...
  try {
    const targetUrl = "https://accsoft.lnctu.ac.in/AccSoft2/Parents/StudentNoDuesFormPrint.aspx";
    let res = await fetch(targetUrl, {
      method: "GET",
      headers: {
        Cookie: cookieString,
        "User-Agent": "Mozilla/5.0",
        Referer: "https://accsoft.lnctu.ac.in/AccSoft2/Parents/StudentNoDuesForm.aspx",
      },
      cache: "no-store",
    });
    ...
    if (html.toLowerCase().includes("please open through no dues form !!") || html.toLowerCase().includes("please open through registration form !!")) {
      // Trigger the intermediate navigation as required by the backend
      const intermediateUrl = "https://accsoft.lnctu.ac.in/AccSoft2/Parents/StudentNoDuesForm.aspx";
    ...
    return new NextResponse(`Failed to load no dues form: ${e.message}`, { status: 500 });
  }
}
