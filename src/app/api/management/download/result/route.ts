...
import { createMgmtClient, MGMT_RESULT_URL, MGMT_DOWNLOAD_REPORT_URL, MGMT_DOWNLOAD_FILE_URL } from "@/lib/ums";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const genHeaders = {
      ...
      "Referer": MGMT_RESULT_URL,
    };

    const params = new URLSearchParams();
    params.append("Enrol_No", downloadId);

    const respGen = await client.post(MGMT_DOWNLOAD_REPORT_URL, params.toString(), {
      headers: genHeaders,
    });
    ...
    const respFile = await client.get(MGMT_DOWNLOAD_FILE_URL, {
      headers: { Referer: MGMT_RESULT_URL },
    ...
    return new NextResponse(respFile.data, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="Result_${downloadId}.pdf"`,
      },
    });
  } catch (e: any) {
    console.error("Result download API error:", e);
