...
import { createMgmtClient, EXAM_FORM_URL, EXAM_DOWNLOAD_REPORT_URL, EXAM_DOWNLOAD_FILE_URL } from "@/lib/ums";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const genHeaders = {
      ...
      "Referer": EXAM_FORM_URL,
    };

    const params = new URLSearchParams();
    params.append("Enrol_No", downloadId);

    const respGen = await client.post(EXAM_DOWNLOAD_REPORT_URL, params.toString(), {
      headers: genHeaders,
    });
    ...
    const respFile = await client.get(EXAM_DOWNLOAD_FILE_URL, {
      headers: { Referer: EXAM_FORM_URL },
    ...
    return new NextResponse(respFile.data, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="ExamForm_${downloadId.substring(0, 10)}.pdf"`,
      },
    });
  } catch (e: any) {
    console.error("Exam Form download API error:", e);
